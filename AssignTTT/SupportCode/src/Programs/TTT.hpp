...
//at different rows and columns
	...
	{
		TTT::State state;
		//int action; //move made to which cell
		int actionR; //moves's row
		int actionC; //moves's column
					 // m_nrCols * r + c -> gives acutal move
					 //in reverse
					 // move % n_cols = c
					 // move / n_colr = r
		int lastMove;
...
//This is a utility function which creates a new state as an exact copy of the state s
	...
	//utility function to generate the successor states
	void GenerateMoves(State s, int color, std::vector<Node> *successors) const;
